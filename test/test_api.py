...
TestinsSesionlocal = sessionmaker(bind=engine_test,autocommit=False,autoflush=False)
...
def insertar_usuario_prueba():
    ...
    engine_test.execute(
        ...
        values
        ('prueba', '{password_hash}','prueba_nombre','prueba_apellido','prueba_direccion',12342,'prueba@gmail.com')
        """
    )
...
def test_crear_usuario():
    usuario = {
        ...
        "apellido": "string",
        "direccion": "string",
        "correo": "anthony@hotsmail.com",
        "telefono": 0,
        "creacion": "2022-08-26T09:15:55.554368"
    }
    ...
    response = cliente.post('/user/', json= usuario,headers= header)
    assert response.status_code == 201
    assert response.json()["respuesta"] == "Usuario creado con exito!!"
...
def test_eliminar_usuario():
    ...
    assert response.json()["respueta"] == "usuario eliminado correctamente!!"
    print(response.json())
...
def test_actualizar_usuario():
    usuario = {
    "username": "anthony12_actualizado",
    }  
    ...
    assert response_user.json()["username"] == "anthony12_actualizado"
